...
init _ url navKey =
    let
        initialModel =
            { navKey = navKey
            , route = UrlParser.parse Route.routeParser url
            , global = Global.init
            , pageModel = Item Item.init
            }
    in
...
trigger an subsequent command, as reflected in the return type of our
...
update msg model =
    ...
    case ( msg, model.pageModel ) of
        ...
        -- received.
        ( UrlChanged url, _ ) ->
            ( { model | route = UrlParser.parse Route.routeParser url }, Cmd.none )
        ...
        -- NOTE: that we almost always discard messages, if they are from a page that
        -- is not active. The reason we catch it in each individual Msg pattern match,
